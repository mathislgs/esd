	...
	if [ $win = 1 ] || [ $draw = 1 ]; then
		...
		if [ "${current_player}" == "X" ]; then # Maximize
			...
			for b in `possible_boards $current_board "X"`; do
				...
				value=$(minimax ${current_depth} $temp_player $b)
				...
				if [ "${symbol}" == "X" ]; then
					...
					for i in `seq 0 8`; do
						...
						if [ ${board:$i:1} = '-' ]; then
							...
							echo "${red}It may take a very long time for the first plays (20min+)${reset}" > /dev/stderr
							value=$(minimax ${depth+1} "O" ${sim_board})
